...
from schemas.users import Users, User, CreateUser, CreateUserSigned
...
def get_user_id(user_id: str, create_user_signed: CreateUserSigned) -> User:
    create_user = CreateUser(**decode_user_token(create_user_signed.signature))
    assert CreateUser(**create_user_signed.dict()) == create_user, "The fields should be the same as the signed one"
    return User(
        id=user_id,
